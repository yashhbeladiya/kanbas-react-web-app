...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      />
      <hr />
      <h3>Updating an Title in an Array</h3>
      ...
      <h3>Updating a status in an Array</h3>
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
        Update Todo</a>
      <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
      <input defaultValue={todo.completed + " "} className="form-control w-50 float-start"
             onChange={(e) => setTodo({ ...todo, completed: e.target.value === "true" }) }/>
      <br /><br /><hr />

      <h3>Updating a Descriptions in an Array</h3>
      <a href={`${API}/${todo.id}/title/${todo.description}`} className="btn btn-primary float-end">
        Update Todo</a>
      <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
